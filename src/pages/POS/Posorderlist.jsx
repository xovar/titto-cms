...
export default function PosOrderList() {
  ...
  useEffect(() => {
    dispatch(fetchOrders({ page: currentPage, limit: 10, channel: 'pos' }));
  ...
  // 🔴 FIX: এখানে strictly 'pos' channel ফিল্টার অ্যাড করা হলো
  const filteredOrders = (items || []).filter((order) => {
    // ১. শুধুমাত্র POS চ্যানেল চেক
    const orderChannel = (order.channel || order.order_channel || '').toLowerCase();
    if (orderChannel !== 'pos') return false;
    ...
    const outletName = (order.outletName || order.outlet_name || '').toLowerCase();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-text-secondary-light dark:text-text-secondary-dark mt-1">
            All in-store sales rung up across your outlets
          </p>
        </div>
      </div>
      ...
      <div className="bg-surface-light dark:bg-surface-dark p-4 rounded-xl border border-border-light dark:border-border-dark space-y-4 shadow-sm">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="relative w-full sm:w-80">
            ...
            <input
              type="text"
              placeholder="Search by ID, Name, Phone or Outlet..."
              value={searchTerm}
          ...
          <button
            onClick={() => dispatch(fetchOrders({ page: currentPage, limit: 10, channel: 'pos' }))}
      ...
      <div className="bg-surface-light dark:bg-surface-dark rounded-xl border border-border-light dark:border-border-dark overflow-hidden shadow-sm">
        {loading ? (
          ...
          <div className="p-12 text-center text-text-secondary-light dark:text-text-secondary-dark">
            ...
            <p className="font-medium text-base">No POS sales found</p>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-border-light dark:divide-border-dark">
                {filteredOrders.map((order) => {
                  ...
                  const firstName = order.firstName || order.first_name || 'Walk-in';
                  const lastName = order.lastName || order.last_name || '';
                  const orderId = order.id || order._id;
                  const outletName = order.outletName || order.outlet_name || '—';
                    ...
                    >
                      ...
                      <td className="px-5 py-4 text-xs text-text-secondary-light dark:text-text-secondary-dark">
                        <div className="flex items-center gap-1.5">
                          <Store size={13} className="text-accent-brand shrink-0" />
                          {outletName}
                        </div>
                      </td>
                      <td className="px-5 py-4 text-xs text-text-secondary-light dark:text-text-secondary-dark capitalize">
                        <div className="flex items-center gap-1.5">
                          <Banknote size={13} className="text-accent-brand shrink-0" />
                          {paymentMethod}
                        </div>
                      </td>
        ...
        {pagination && pagination.totalPages > 1 && (
          <div className="p-4 border-t border-border-light dark:border-border-dark flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-text-secondary-light dark:text-text-secondary-dark">
            <div>
              ...
              <span className="font-semibold">{pagination.total}</span> sales)
            </div>
